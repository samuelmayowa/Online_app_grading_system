...
if(isset($_POST['proceed'])){
    ...
    if(!empty($rrdataPaid)){
        ...
        $fname=$fullname[2]?$fullname[2]:$regnum;
        $midname = $fullname[1]?$fullname[1]:NULL;
        $lname= $fullname[0]?$fullname[0]:NULL;
        $email = $screenData['email'];
        $username = $regnum;
        $password = $regnum;
        $phno = '0'.$screenData['phone'];
        ...
        if($usr=mysqli_query($con,"SELECT Username From userAccounts Where Username ='$username'")){
            ...
            $msg="Username Already Taken";
                ...
                $query=mysqli_query($con,"INSERT INTO userAccounts (FirstName, MidName, LastName, Email,Username , Password, PhoneNumber, type)
                                        VALUES('$fname','$midname', '$lname', '$email', '$username', '$password', '$phno', 'putme')");
                if(!$query){
                    echo  mysqli_error();
                } else {
                   $_SESSION['user'] = $username;
                   header('location:login.php?id='.$username . ' && msg=Your Account Created Successfully, your password is your jamb reg number. Thank you!');
                }
            }
        }
    }elseif(!empty($rrdata)){
                                ...
                                <div class="card-header"><h3 class="text-center font-weight-light my-4" style="color:#28a745; font-family:Arial Black;"> UTME Candidates Screening / Verification </h3></div>
                                    <div class="card-body">
                                        <div class="card-header">
                                            <p>Not paid for PUTME? <a class="btn btn-primary" href="sign_in.php">Pay PUTME</a></p><h6 class="text-center font-weight-light my-4" style="color:#23c745; font-family:Arial Black;"> Verify Your Payment With Your UTME Registration Number</h6></div>
                                        <form  Method="POST" id="jambform">
                                            ...
                                            <div class="form-group d-flex align-items-center justify-content-between mt-4 mb-0">
                                                ...
                                                <button class="btn btn-primary" id="submit" name="login" type="submit">Verify My Payment</button>
                                                <div id="info"> </div>
                                            </div>
                                        </form>
                                    ...
                                    <div class="card-footer text-center">
                                        <div class="small" style="color:maroon;">
                                            <a href="uploadverify.php" style="color:maroon;"> Already Verified and Paid(for UTME)? (Create User Account Here) Or Sign up! &nbsp; &nbsp; &nbsp;</a> || &nbsp; &nbsp; &nbsp;
	...
	$(document).ready(function(){
		$("#myModal").modal('show');
         ...
         $('#jambform').submit(function(e){
                    ...
                    var info = $("#info");
        info.html('<div class="loading-indicator"><span><img src="https://putme.escohsti-edu.ng/images/load.gif" width="40" height="40"/></span><p>Loading...</p></div>');
                $("#submit").prop('disabled', true);
                      
        $.ajax({   
                            url:"/verifypay.php?regnum="+regnum,
                            type: 'POST',
                            dataType: 'json',
                            beforeSend: function() {},
                            timeout:10000,
                            ...
                            success: function(response) {
                               ...
                               // alert("Deposit Placed");
                                ...
                                setTimeout(function(){ 
                                    ...
                                    var email =  '<div class="form-group"><label class="small mb-1" for="Email">Email:  &nbsp; </label><input class="form-control py-4" type="email" readonly placeholder="Email"value="'+response.userdata.email+'" id="email"name="email"  required /></div>';
                                    ...
                                    var sub = '<div class="form-group d-flex align-items-center justify-content-between mt-4 mb-0"><button class="btn btn-primary"  name="proceed" type="submit">Create Account</button></div>';
                                ...
                                }else if(check == 'paid' && status == false){
                                     setTimeout(function() { 
                                        $("#response").html('');
                                ...
                                }else if(check === 'not_paid_yet' && status === false){
                                    setTimeout(function() {
                                        window.location.replace("/putme_invoice.php?regnum="+regnum);
                                    }, 5000);
                                }else if(check == 'not-gen' && status == false){
                                    setTimeout(function() {
